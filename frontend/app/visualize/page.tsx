...
const tooltipItemStyle = { color: "#ffffff", fontWeight: "600" };
const tooltipLabelStyle = { color: "#a78bfa", fontWeight: "700", marginBottom: "4px" };
...
export default function VisualizePage() {
    ...
    const scatterData = dash?.trends?.map((t, i) => ({
        x: t.id, y: t.value, z: Math.abs(Math.sin(i) * 100),
    })) ?? [];
    ...
    const composedData = dash?.trends?.slice(0, 20).map((t) => ({
        name: `${t.id}`,
        area: t.value,
                        ...
                        <div className="glass-card p-6 rounded-xl">
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                                <AreaChart data={dash.trends} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                                    ...
                                    <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} labelStyle={tooltipLabelStyle} cursor={{ fill: "rgba(255,255,255,0.05)" }} />
                            ...
                            <ResponsiveContainer width="100%" height={260}>
                                <BarChart data={dash.category_data} layout="vertical" margin={{ top: 4, right: 20, left: 10, bottom: 0 }}>
                                    ...
                                    <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                                        {dash.category_data.map((_, i) => (<Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
